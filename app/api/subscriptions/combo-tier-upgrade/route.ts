...
import { COMBO_MAX_INSTALLMENTS } from '@/lib/checkout/combo-billing';
import {
  upgradeComboPlanTierViaAsaas,
  upgradeComboPlanTierViaPagarme,
...
const bodySchema = z
  .object({
    ...
    targetPlanSlug: z.enum(['aventureiro', 'heroi', 'lendario']),
    installmentCount: z
      .number()
      .int()
      .min(1)
...
export async function POST(request: Request) {
  ...
  gatewaySubscription.pagarme_customer_id
    ? 'pagarme'
    : gatewaySubscription.asaas_subscription_id ||
        gatewaySubscription.asaas_customer_id
      ? 'asaas'
      : null;

  if (provider === 'asaas' && !ASAAS_CONFIGURED) {
    return NextResponse.json(
      { error: 'Asaas não configurado para upgrade de combo.' },
      { status: 503 }
    );
  }
  ...
  if (provider === 'pagarme' && !PAGARME_CONFIGURED) {
    return NextResponse.json(
      { error: 'Pagar.me não configurado para upgrade de combo.' },
      { status: 503 }
    );
  }
  ...
  if (provider !== 'asaas' && provider !== 'pagarme') {
    return NextResponse.json(
      { error: 'Upgrade de combo indisponível para esta assinatura.' },
      { status: 400 }
    );
  }
  ...
  try {
    if (provider === 'pagarme') {
      ...
      const result = await upgradeComboPlanTierViaPagarme({
        supabase,
        userId: user.id,
        subscriptionId: body.subscriptionId,
        targetPlanSlug: body.targetPlanSlug,
    ...
    const result = await upgradeComboPlanTierViaAsaas({
      supabase,
      userId: user.id,
      subscriptionId: body.subscriptionId,
      targetPlanSlug: body.targetPlanSlug,
    ...
    console.error('[combo-tier-upgrade] api:', error);
    return NextResponse.json(
      {
        error:
          provider === 'pagarme'
