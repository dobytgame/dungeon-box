...
import { syncPagarmeSubscriptionAction } from '@/lib/admin/actions';
...
export default function SyncPagarmeButton({ subscriptionId, compact }: Props) {
    ...
    <div className={compact ? 'inline-block' : 'space-y-2'}>
      <button
        ...
        onClick={(event) => {
          ...
          void syncPagarmeSubscriptionAction(subscriptionId).then((result) => {
            setPending(false);
            ...
            if ('success' in result && result.success) {
              setMessage(
                result.reconciled
                  ? 'Assinatura pendente ativada via Pagar.me.'
      ...
      >
        {pending ? 'Sincronizando…' : compact ? 'Pagar.me' : 'Sincronizar Pagar.me'}
      </button>
      {!compact && error ? (
